...
export function ParentPhotoSlider({ compact = false, audience = "parent" }: { compact?: boolean; audience?: "parent" | "teacher" }) {
  ...
  useEffect(() => { const timer = window.setInterval(() => setSlideIndex((index) => (index + 1) % slides.length), 5_000); return () => window.clearInterval(timer); }, [slides.length]);
  return <aside className={`parent-photo-slider ${compact ? "parent-photo-slider-compact" : ""}`} aria-label="Moments from TribePetra Kids">
    <Image alt="" className="parent-photo-slider-image" fill key={slides[slideIndex]} priority={slideIndex === 0} sizes="(max-width: 920px) 100vw, 48vw" src={`${folder}/${slides[slideIndex]}`} />
