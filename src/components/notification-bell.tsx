...
export function NotificationBell({ serviceSessionId }: { serviceSessionId?:number }) {
  ...
  return <div className="notification-wrap">
    ...
    `}</style>
    <style jsx global>{`
      .notification-item small{overflow-wrap:anywhere}.notification-item>span{min-width:0}.notification-item.roster{background:linear-gradient(90deg,#f7fbff,#fffdfa)}.notification-item.roster>i{background:#eef0ff;color:#5b49c6}
    `}</style>
  </div>;
}
