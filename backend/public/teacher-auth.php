...
function teacher_issue_verification(PDO $db, int $staffId, string $whatsapp, string $name): ?string {
    ...
    $db->prepare('INSERT INTO staff_whatsapp_verifications(staff_user_id,token_hash,expires_at) VALUES(?,?,DATE_ADD(NOW(), INTERVAL 24 HOUR))')->execute([$staffId, hash('sha256', $token)]);
    $url = teacher_request_origin() . '/teacher/verify?token=' . rawurlencode($token);
    tpk_send_whatsapp_verification($whatsapp, $name, $url);
    return tpk_app_environment() === 'development' ? $url : null;
}
...
if ($method === 'POST' && $path === '/api/teachers/register') {
    ...
    $response = ['message' => 'We sent a verification link to your WhatsApp number. Verify your account to sign in.'];
    if ($developmentUrl) $response['developmentVerificationUrl'] = $developmentUrl;
