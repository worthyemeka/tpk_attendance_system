...
function checkin_guardian(PDO $db,string $phone): ?array { foreach($db->query('SELECT id,family_id,phone FROM guardians')->fetchAll() as $guardian) if(checkin_phone($guardian['phone'])===$phone) return $guardian; return null; }
...
try {
    ...
    audit($db,(int)$session['campus_id'],'PUBLIC_CHILDREN_CHECKED_IN','ServiceSession',(int)$session['id'],['guardianId'=>(int)$guardian['id'],'childIds'=>$childIds]);$db->commit();checkin_response(['serviceSessionId'=>(int)$session['id'],'pickupCode'=>'TPK-'.strtoupper(bin2hex(random_bytes(3))),'checkedIn'=>$checked,'classAssignmentRequired'=>$pending]);
